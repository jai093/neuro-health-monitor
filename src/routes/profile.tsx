...
import { FormSkeleton, LoadErrorCard } from "@/components/page-skeletons";
...
function ProfilePage() {
  const { state, hydrated, loadError, retry, saveProfile, clearAll } = useNeuro();
  ...
  const submit = () => {
    ...
    try {
      ...
      toast.error("Couldn't save your profile", {
        description: "This device's storage may be full or blocked.",
  ...
  const wipe = () => {
    try {
      ...
      toast.error("Couldn't clear your data", { action: { label: "Retry", onClick: wipe } });
    }
  };
  ...
  return (
    <AppShell>
      <PageHeader
        eyebrow="Account"
        title="Your Profile"
        description="Stored only in this browser and kept in sync live across tabs. Clearing your data removes every assessment permanently."
      />
